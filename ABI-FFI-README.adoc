...
chez/node/racket/refc), so the older `+idris2 --cg c-header+` step is
aspirational — update the header by hand in the same commit as any FFI
signature change. See
link:docs/ABI-FFI-BOUNDARY.md[`+docs/ABI-FFI-BOUNDARY.md+`].
...
. *Update the C header by hand* (`+ffi/zig/include/valence_shell.h+`) to
match the new/changed `+export fn+` signatures — Idris2 has no
`+c-header+` codegen.
...
* link:docs/ABI-FFI-BOUNDARY.md[ABI/FFI boundary decision record]
