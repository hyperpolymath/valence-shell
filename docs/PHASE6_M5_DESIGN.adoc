...
*Status*: In Progress *Version*: 0.9.0 *Date*: 2026-01-29
*Prerequisites*: Phase 6 M4 (Variables) ✅ Complete

=== Overview

Implement POSIX-compliant quote parsing for proper string handling and
argument separation.

*Goals*: - Single quotes (`+'...'+`) - literal strings, no expansion -
Double quotes (`+"..."+`) - expansion allowed, preserves spaces -
Backslash escaping (`+\+`) - Quote-aware variable expansion - Positional
parameters (`+$1+`, `+$2+`, `+$@+`, `+$*+`) - Proper argument splitting

*Timeline*: 2-3 weeks *Complexity*: High (quote nesting, escaping rules,
edge cases)
...
*Rules*: - Everything between `+'+` and `+'+` is literal - Cannot escape
`+'+` inside single quotes - No variable expansion - Preserves all
whitespace
...
*Rules*: - Variables expand (`+$VAR+`, `+${VAR}+`) - Special variables
expand (`+$?+`, `+$$+`) - Command substitution expands (future:
`+$(cmd)+`) - Preserves whitespace - Can escape with backslash: `+\"+`,
`+\$+`, `+\\+`
...
*Rules*: - `+\<newline>+` - line continuation - `+\$+` - literal dollar
- `+\\+` - literal backslash - `+\+` - literal space (prevents word
splitting) - Inside double quotes: only `+\"+`, `+\$+`, `+\\+`, `+\n+`
escape
...
*Key changes*: - Track quote state during tokenization - Build words
from multiple `+TokenPart+`s - Handle backslash escaping in all contexts
...
*Special variables to add*: - `+$0+` - Shell name ("`vsh`") - `+$1+`,
`+$2+`, …, `+$9+` - Positional parameters - `+$#+` - Number of
positional parameters - `+$@+` - All parameters as separate words -
...
[width="100%",cols="25%,51%,24%",options="header",]
|===
...
|`+echo 'don'\''t'+` |`+["echo", "don't"]+` |Escaping in single quotes
(workaround)

|`+echo "say \"hi\""+` |`+["echo", "say \"hi\""]+` |Escaped double
quotes
...
[arabic]
. *Unclosed quotes*: `+echo "hello+` → Error
. *Nested quotes*: `+echo "it's fine"+` → OK (single quote inside
double)
. *Empty arguments*: `+echo "" ''+` → `+["echo", "", ""]+`
. *Backslash at EOL*: `+echo hello\<newline>world+` → Line continuation
. *Mixed quoting*: `+echo "Hello "'$NAME' "$USER"+` → Complex parsing
. *Unicode*: `+echo "日本語"+` → UTF-8 safe
. *Special chars in quotes*: `+echo "!@#$%"+` → All literal
