...
*Version*: 0.12.0 *Status*: In Progress *Date*: 2026-01-29

'''''

=== Overview

Implement POSIX arithmetic expansion using `+$((expression))+` syntax.
This allows integer arithmetic operations within command arguments and
...
The arithmetic expansion can be nested. To specify nesting, the inner
expansion shall be enclosed in parentheses.
...
. `+! ~ + -+` - Logical NOT, bitwise NOT, unary plus/minus
...
*Integer Arithmetic:* - All values are *signed integers* (i64 in Rust) -
Division by zero is an error - Overflow behavior: wrap around (Rust
default)
...
[arabic]
. *Whitespace handling*: `+$(( 5 + 3 ))+` same as `+$((5+3))+`
. *Nested parentheses*: `+$(( (( 5 + 3 )) * 2 ))+`
. *Variable names*: `+$((foo))+`, `+$((foo_bar_123))+`
. *Undefined variables*: Treat as 0 (POSIX behavior)
. *Overflow*: Wrap around (Rust i64 behavior)
. *Division by zero*: Error with message
. *Negative exponents*: Error (not supported in POSIX)
. *Shift overflow*: Undefined behavior (limit shift amount?)
...
. *Ternary operator*: `+$(( x > 0 ? 1 : -1 ))+`
. *Octal/hex literals*: `+$((010))+` (octal), `+$((0x10))+` (hex)
. *Comma operator*: `+$(( x = 5, y = 10, x + y ))+`
...
*Benefits*: - ✅ Division by zero proven impossible (dependent types) -
✅ Overflow behavior proven correct - ✅ Operator precedence proven to
match POSIX - ✅ All edge cases formally verified

See: `+proven/src/Proven/SafeArith.idr+` (to be created)

'''''

*Next Steps:* 1. Create `+src/arith.rs+` with evaluator 2. Add
arithmetic parsing to `+parser.rs+` 3. Integrate with expansion pipeline
4. Write comprehensive tests 5. Update version to 0.12.0
