...
*You can contribute without knowing proof assistants!* This guide shows
you how.
...
*Tier 1 (You)*: Features, bug fixes, tests (no proofs) *Tier 2*: Core
modifications (some spec awareness) *Tier 3*: Verification experts
(proof updates)
...
*IntelliJ IDEA*: - Install Rust plugin - Import as Cargo project
...
This creates: - `+impl/rust-cli/src/commands/chmod.rs+` (skeleton) -
...
*Why?* Property tests are derived from the formal specifications. If
your Rust code passes the property tests, it probably matches the
proofs.
...
Look at similar code: - `+impl/rust-cli/src/commands/mkdir.rs+` -
Reversible operation example - `+impl/rust-cli/src/builtins.rs+` -
Built-in command example - `+impl/rust-cli/tests/property_tests.rs+` -
...
Ask questions:
https://github.com/hyperpolymath/valence-shell/discussions
...
. *Verification* (Only if needed, Tier 2/3):
...
[arabic]
. Add reversible operations (chmod, ln -s)
. Modify parser
...
https://github.com/hyperpolymath/valence-shell/pull/123[PR #123: Add pwd
command] (Simple) -
https://github.com/hyperpolymath/valence-shell/pull/145[PR #145: Add
chmod operation] (Medium) -
https://github.com/hyperpolymath/valence-shell/pull/167[PR #167: Improve
error messages] (Easy)
...
[arabic]
. Read: `+docs/ARCHITECTURE.md+` (understand the codebase)
. Browse: Open issues labeled "`good first issue`"
. Try: Add a simple command (pwd, echo, etc.)
