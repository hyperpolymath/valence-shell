...
*Status*: Design Complete, Implementation Pending (ROADMAP v2.0 — Idris2
path) *Version*: 0.1.0 *Updated*: 2026-01-28 (status last reviewed
2026-06-01)
...
This document describes how to extract OCaml code from Lean 4 proofs for
the Valence Shell project.

*Key Insight*: Lean 4 compiles to C, not OCaml directly. We use C as an
intermediate layer with OCaml FFI bindings.
...
|*Extraction Definition* |`+proofs/lean4/Extraction.lean+` |Export
interface
...
|*Integration* |`+impl/ocaml/filesystem_ffi.ml+` |Combines verified +
real ops
|===
...
*Output*: - `+.lake/build/lib/Extraction.o+` - Compiled object files -
...
*Flags explained*: - `+-shared+` - Create shared library (.so) -
...
`+-lleanshared+` - Link against Lean runtime - `+-Wl,-rpath,...+` -
Embed runtime library path in .so
...
The Lean `+Filesystem+` type is an abstract function
`+Path → Option FSNode+`. For real operations, we need to:

[arabic]
. *Track state in Lean object*: Update Lean filesystem value after each
operation
. *Sync with real filesystem*: Ensure Lean state matches disk state
. *Handle divergence*: Deal with external modifications to filesystem

*Design Decision*: Use Lean only for precondition checking, not state
tracking.
...
*FFI Overhead*: - OCaml → C: ~10-50ns per call - C → Lean: Negligible
(already compiled) - String marshaling: ~100ns per path

*Optimization*: Cache precondition check results when paths haven’t
changed.
...
*Advantage*: Direct OCaml output *Disadvantage*: Lean 4 is our primary
...
*Advantage*: Full control, no FFI overhead *Disadvantage*: No formal
...
The Rust CLI (`+impl/rust-cli/+`) is the primary implementation. OCaml
extraction serves as:

[arabic]
. *Reference implementation*: Directly from proofs
. *Cross-validation*: Compare Rust vs OCaml behavior
. *Audit tool*: Verify Rust CLI matches specification
...
* link:++$(lean%20--print-prefix)/include/lean/lean.h++[Lean Runtime
API]
* https://github.com/ocamllabs/ocaml-ctypes[OCaml Ctypes Tutorial]
* https://sel4.systems/[seL4 Verification] - Similar approach (Isabelle
→ C)
...
For questions about OCaml extraction: - Check:
`+proofs/lean4/Extraction.lean+` (inline documentation) - Check:
`+impl/ocaml/lean_wrapper.c+` (implementation notes) - Repo:
