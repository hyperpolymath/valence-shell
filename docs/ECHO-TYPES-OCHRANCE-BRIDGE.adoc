...
* RMO: Remove-Match-Obliterate, an abstract irreversible transition. The
  `obliterate` command is implemented and wired as a best-effort 3-pass
  overwrite + unlink + unsigned audit residue. HMAC chaining exists in the
  audit-log library, but RMO key provisioning, external anchoring, storage-level
  erasure, and legal compliance are incomplete.
...
* Do not claim RMO/GDPR support beyond the implemented best-effort `obliterate`
  (cooperating in-place filesystems only; no RMO key provisioning, external
  audit anchor, or verified device erase) — i.e. do not claim a complete GDPR
