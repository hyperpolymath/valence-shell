...
Implement POSIX shell variable support including: - Variable assignment
and retrieval - Variable expansion in commands - Environment variable
access - Special parameters ($?, $0, $latexmath:[, etc.)
...
*Syntax Rules:* - No spaces around `+=+` - Value can be unquoted (single
word) or quoted - Assignment can appear before commands:
`+VAR=x cmd args+`
...
*Expansion Contexts:* - Command arguments - Arithmetic expressions:
`+$((x + $VAR))+` - Here documents (unless delimiter quoted) -
...
. Array variables (bash extension, defer)
...
*Identifier Rules:* - Start with letter or underscore - Followed by
...
*Assignment Detection:* - Occurs at start of command - Or before
command: `+VAR=x cmd args+`
...
[arabic]
. *Identify variable references* in input:
...
* [ ] Variables work in all contexts (commands, redirections,
arithmetic)
...
* PHASE6_M7-M10_COMPLETE.md - Process substitution, arithmetic, here
docs, job control
...
. *M13: Quote Processing* - Full quote removal and escape handling
. *Phase 4: Verification* - Prove Rust implementation matches Lean
theorems
