...
*Since:* v1.1.0 *Spec:* bash-compatible parameter expansion syntax

=== Overview

Parameter expansion allows you to manipulate variables inline using the
`+${VAR...}+` syntax. This provides powerful text manipulation
capabilities without external tools.
...
|`+${VAR-default}+` |Use default if unset only |`+${EMPTY-fallback}+` →
``+(empty) | |+`latexmath:[{VAR:=default}` | Assign default if unset/null | Returns default value* |
| `]\{VAR=default}`+| Assign default if unset only | Returns default value* | |+`latexmath:[{VAR:+value}` | Use value if set/non-null | `]\{VAR:+yes}`+→+`yes`+(if VAR set) | |+`latexmath:[{VAR+value}` | Use value if set | `]\{EMPTY+yes}`+→+`yes`+| |+`latexmath:[{VAR:?message}` | Error if unset/null | Prints error, returns empty |
| `]\{VAR?message}`+| Error if unset | Prints error, returns empty | |+`latexmath:[{#VAR}` | String length | `]\{#PATH}`+→+`42`+| |+`latexmath:[{VAR:offset}` | Substring from offset | `]\{VAR:5}`+→ substring from index 5 | |+`latexmath:[{VAR:offset:length}` | Substring with length | `]\{VAR:5:3}`+→ 3 chars from index 5 | |+`latexmath:[{VAR: -offset}` | Substring from end | `]\{VAR:
-5}` → last 5 characters
|===

_Note: Assignment (`+:=+`, `+=+`) deferred to v1.1.1 - currently returns
value without persisting assignment_
...
*With colon (`+:+`)*: Checks for both unset AND null (empty) *Without
colon*: Only checks for unset
...
*Important*: Negative offset requires space: `+${VAR: -5}+` not
`+${VAR:-5}+` (which is default value)
...
* ⚠️ `+${VAR:=default}+` - *Returns default but doesn’t assign in
v1.1.0*
** Assignment deferred to v1.1.1 (requires mutable state in
expand_variables)
** Currently behaves like `+${VAR:-default}+` (returns default without
assigning)
...
* ❌ Array expansions (`+${arr[@]}+`, `+${!arr[@]}+`, etc.) - Coming in
v1.1.0
...
* Unicode character counting may be slower than byte counting for very
large strings
...
Current test coverage: 67+ tests covering all operators, edge cases,
Unicode, and special variables.

=== See Also

* https://www.gnu.org/software/bash/manual/html_node/Shell-Parameter-Expansion.html[Bash
Reference Manual - Shell Parameter Expansion]
* https://pubs.opengroup.org/onlinepubs/9699919799/utilities/V3_chap02.html#tag_18_06_02[POSIX
Parameter Expansion]
