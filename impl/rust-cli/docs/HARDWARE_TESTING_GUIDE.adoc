...
You need a spare drive that: - Is NOT your system drive - Contains NO
important data - Is either: - External USB drive - Secondary internal
drive - Old SSD/HDD you’re willing to destroy
...
[arabic]
. *System drive protection* - Try to erase system drive (should ABORT)
. *Mount detection* - Try with mounted device (should warn and offer
unmount)
. *Typed challenge* - Type wrong device name (should CANCEL)
. *Final confirmation* - Answer '`n`' to final prompt (should CANCEL)
. *Capability detection* - Try unsupported method (should error
gracefully)
...
Would you like me to: 1. Add a `+hardware_erase+` command to the Rust
CLI for easier testing? 2. Create a test script that uses a loop device
(safe, no real hardware needed)? 3. Add detailed logging to track what
happens during erase operations?

*Current recommendation*: Start with the *Eclipse drive (/dev/nvme0n1)*
if it doesn’t have critical data, since you mentioned it’s for
repos/data storage. But verify no important repos are on it first!
