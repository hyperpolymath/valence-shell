...
*Date*: 2026-01-28 (status last reviewed 2026-06-01) *Status*: Design
Complete, Implementation Pending (ROADMAP v2.0 — Idris2 path)

'''''

=== What Was Built

A complete *Lean 4 → C → OCaml/Zig* extraction pipeline enabling
formally verified code to be called from both OCaml and Zig.
...
*Files*: - `+docs/OCAML_EXTRACTION.md+` (~600 lines) -
...
* Zig via C interop
* Extensible to any language with C FFI (Python, Ruby, Node.js, Go,
Rust)
...
*TODO*: 1. Implement Lean object creation in `+vsh_filesystem_create()+`
2. Add actual Lean function calls in `+vsh_safe_*()+` functions 3.
Properly marshal Lean CResult to C vsh_result_t 4. Handle Lean object
reference counting
...
[arabic]
. Update `+impl/ocaml/filesystem_ffi.ml+` to use verified preconditions
. Update `+impl/zig/src/lib.zig+` to use lean_bindings
...
*Mitigation*: - Thorough code review - Comprehensive testing - Runtime
assertions - Fuzzing - Property-based tests
...
*Recommendation*: Use Lean verification for production systems where
correctness is critical (financial, medical, safety-critical).
...
[width="100%",cols="18%,28%,34%,20%",options="header",]
|===
...
[arabic]
. *Polyglot Verification*: 6 proof systems (Coq, Lean 4, Agda, Isabelle,
Mizar, Z3)
. *Multi-Language Target*: OCaml + Zig + (any C FFI language)
. *Optional Verification*: Build flag to enable/disable
. *Lightweight*: <1MB overhead, <10ms cold start
...
* https://github.com/ocamllabs/ocaml-ctypes[OCaml Ctypes]
* https://ziglang.org/documentation/master/#C[Zig C Integration]
* https://sel4.systems/[seL4 Verification]
* https://compcert.org/[CompCert]
...
*Proof Systems*: Lean 4, Coq, Agda, Isabelle, Mizar, Z3 *Languages*:
OCaml, Zig, C *License*: MPL-2.0

'''''

=== Getting Started

*For OCaml Users*: See `+docs/OCAML_EXTRACTION.md+` *For Zig Users*: See
`+docs/ZIG_LEAN_EXTRACTION.md+` *For Contributors*: See inline
documentation in source files

'''''

*Status*: Ready for implementation. Complete lean_wrapper.c TODOs to
