...
| `rmo_operations.smt2`
| Abstract Remove-Match-Obliterate tree/block-mapping properties. These do not
  model storage hardware, crash durability, the Rust implementation, or legal
  compliance.
|===
...
The verifier checks the expected result sequence for every script, rejects Z3
diagnostics even when the process exits successfully, and includes a positive
failure control. The `sat` checks in `filesystem_operations.smt2` and
`symlink_operations.smt2` are model-consistency checks, not proved theorems.
