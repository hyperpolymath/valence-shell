...
-->
# Alkahest ABI/FFI Documentation
...
│       └── include/
│           └── alkahest.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── alkahest.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/alkahest.h
```
...
int main() {
    void* handle = alkahest_init();
    if (!handle) return 1;

    int result = alkahest_process(handle, 42);
    if (result != 0) {
        const char* err = alkahest_last_error();
...
Compile with:
```bash
gcc -o example example.c -lalkahest -L./zig-out/lib
```

### From Idris2

```idris
import Alkahest.ABI.Foreign
...
#[link(name = "alkahest")]
extern "C" {
    fn alkahest_init() -> *mut std::ffi::c_void;
    fn alkahest_free(handle: *mut std::ffi::c_void);
    fn alkahest_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = alkahest_init();
        assert!(!handle.is_null());

        let result = alkahest_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libalkahest = "libalkahest"

function init()
    handle = ccall((:alkahest_init, libalkahest), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:alkahest_process, libalkahest), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:alkahest_free, libalkahest), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/alkahest.h
   ```
